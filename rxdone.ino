...
void loop() {
  if (radio.available()) {
    ...
    int gasLevel = parseGasLevel(text);
    float temperature = parseTemperature(text);
    float humidity = parseHumidity(text);
    int pulseRate = parsePulseRate(text);
...
//    Serial.println("Pulse Rate: " + String(pulseRate));
    ...
    sendToThingSpeak(gasLevel, temperature, humidity, pulseRate);
  }
}
...
void sendToThingSpeak(int gasLevel, float temperature, float humidity, int pulseRate) {
  HTTPClient http;

  // Your ThingSpeak update URL
  String url = "http://api.thingspeak.com/update?api_key=" + String(api_key) +
               "&field1=" + String(gasLevel) +
               "&field2=" + String(temperature) +
               "&field3=" + String(humidity) +
               "&field4=" + String(pulseRate);
